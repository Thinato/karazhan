#!/bin/sh
# Fetches the latest karazhan release and drops the binary somewhere on disk.
#
#   curl --proto '=https' --tlsv1.2 -sSf \
#     https://raw.githubusercontent.com/Thinato/karazhan/main/install.sh | sh
#
# Knobs (env vars):
#   KARAZHAN_INSTALL_DIR   where the binary lands. Defaults to $HOME/.local/bin.
#   KARAZHAN_VERSION       release tag to grab (default: latest, e.g. v0.2.0).

set -eu

REPO="Thinato/karazhan"
BIN="karazhan"
DEST_DIR="${KARAZHAN_INSTALL_DIR:-$HOME/.local/bin}"
TAG="${KARAZHAN_VERSION:-latest}"
